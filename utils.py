...
def compute_loss(p, targets, model):  # predictions(网络输出), targets, model
                                      # targets = [image, class, x, y, w, h]
                                      # image表示batch中图片编号 class表示类别 x y w h就是box信息
    ft = torch.cuda.FloatTensor if p[0].is_cuda else torch.Tensor       # 若p[0]在gpu上，则在gpu上（.cuda）新建一个类型为FloatTensor的张量，否则在cpu上新建
    lcls, lbox, lobj = ft([0]), ft([0]), ft([0])
    tcls, tbox, indices, anchor_vec = build_targets(p, targets, model)     # 筛选正样本，并且将anchor与正样本对应上 且正样本的box信息映射到了每一层特征图上
        # tcls:   yolov3的三层中存下来的正样本的类别号
                  # class  size(n) 长度为4的tuple，每个元素都是torch.size([n])的tensor
        # tbox:   正样本存下来的在对应特征图上的相对自己栅格的box中心浮点坐标以及对应的box框
                  # gxy+gwh，即xywh(grid)，且gxy减掉了整数部分 size(n,4)
        # av:     正样本对应的不同层的anchor的box的宽高值（anchor真值除32 16 8）
                  # anchor_vec size(n,4)
        # indice: 索引b, a, gj, gi（image, anchor, gridy, gridx），对应yolo_layer的输出维度
                  # b: 一个batch中的下标
    ...
    # 设为"sum"表示对样本进行求损失和；设为"mean"表示对样本进行求损失的平均值；设为"none"表示对样本逐个求损失，输出与输入的shape一样
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions  # i是层编号 pi为第i层特征图,对应YOLO层的所有实际预测结果
        b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx  #第i层正样本的batch编号 anchor 以及栅格坐标
        ...
        if nb:  # number of targets
            ...
            ps = pi[b, a, gj, gi]  # prediction subset corresponding to targets  # 维度 (nb, 9)
            # ps[:, 2:4] = torch.sigmoid(ps[:, 2:4])  # wh power loss (uncomment)

            # GIoU
            # 计算中心坐标 用sigmoid将tx,ty压缩到[0,1]区间內，可以有效的确保目标中心处于执行预测的网格单元中，防止偏移过多
            # 预测出来的是一个偏移量 不是绝对坐标值
            # 网络不会预测边界框中心的确切坐标而是预测与预测目标的grid cell左上角相关的偏移tx,ty
            pxy = torch.sigmoid(ps[:, 0:2])  # pxy = pxy * s - (s - 1) / 2,  s = 1.5  (scale_xy)
            pwh = torch.exp(ps[:, 2:4]).clamp(max=1E3) * anchor_vec[i]  # 计算正样本box框的 w, h
...
def compute_loss_4p(p, targets, model):  # predictions(网络输出), targets, model
                                        # targets = [image, class, x, y, w, h]
                                        # image表示batch中图片编号 class表示类别 x y w h就是box信息
    ft = torch.cuda.FloatTensor if p[0].is_cuda else torch.Tensor  # 若p[0]在gpu上，则在gpu上（.cuda）新建一个类型为FloatTensor的张量，否则在cpu上新建
    lcls, lbox, lobj, lmse = ft([0]), ft([0]), ft([0]), ft([0])
    tcls, tbox, indices, anchor_vec = build_targets_4p(p, targets, model)  # 筛选正样本，并且将anchor与正样本对应上 且正样本的box信息映射到了每一层特征图上
        # tcls:   yolov3的三层中存下来的正样本的类别号
            # class  size(n) 长度为4的tuple，每个元素都是torch.size([n])的tensor
        # tbox:   正样本存下来的在对应特征图上的相对自己栅格的box中心浮点坐标以及对应的box框
            # gxy+gwh，即xywh(grid)，且gxy减掉了整数部分 size(n,12)
        # av:     正样本对应的不同层的anchor的box的宽高值（anchor真值除32 16 8）
            # anchor_vec size(n,4)
        # indice: 索引b, a, gj, gi（image, anchor, gridy, gridx），对应yolo_layer的输出维度
            # b: 一个batch中的下标
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions  # i是层编号 pi为第i层特征图,对应YOLO层的所有实际预测结果
        ...
        if nb:  # number of targets
            ...
            pxy = torch.sigmoid(ps[:, 0:2])  # pxy = pxy * s - (s - 1) / 2,  s = 1.5  (scale_xy)
            # pwh = torch.exp(ps[:, 2:4]).clamp(max=1E3) * anchor_vec[i]  # 计算正样本box框的 w, h
            pwh = torch.exp(ps[:, 2:10]).clamp(max=1E3) * torch.cat((anchor_vec[i], anchor_vec[i], anchor_vec[i], anchor_vec[i]), 1) / 2
            px = torch.cat((pxy[:, 0:1]-pwh[:, 0:1], pxy[:, 0:1]+pwh[:, 2:3], pxy[:, 0:1]+pwh[:, 4:5], pxy[:, 0:1]-pwh[:, 6:7]), 1)
            py = torch.cat((pxy[:, 1:2]-pwh[:, 1:2], pxy[:, 1:2]-pwh[:, 3:4], pxy[:, 1:2]+pwh[:, 5:6], pxy[:, 1:2]+pwh[:, 7:8]), 1)
            pxmax, index1 = torch.max(px, 1)
            pxmin, index2 = torch.min(px, 1)
            pymax, index3 = torch.max(py, 1)
            pymin, index4 = torch.min(py, 1)
            pm = torch.cat((pxmax.view(-1, 1)-pxmin.view(-1, 1), pymax.view(-1, 1)-pymin.view(-1, 1)), 1)

            # 合成完整的box框信息，带中心坐标 带w h
            # pbox 是预测函数  anchor是初值， 之间的变换参数就是要训练出的回归系数
            # 训练的目的就是让pbox无限接近真值，得到这组无限接近真值时的系数  这就是边框回归的核心
            pbox = torch.cat((pxy, pm), 1)  # predicted box
            # 下面就是构造损失函数让预测结果通过怎样的优化去更接近真值了 构造损失函数后就定义了对应关系了啊
            # 计算GIOU部分
            giou = bbox_iou(pbox.t(), tbox[i], x1y1x2y2=False, GIoU=True)  # giou computation
            # 计算giou的损失值  box的loss是1-giou的值
            ...
            # 计算角点回归的mse损失
            pxiyi = torch.cat((px[:, 0:1], py[:, 0:1], px[:, 1:2], py[:, 1:2], px[:, 2:3], py[:, 2:3], px[:, 3:4], py[:, 3:4]), 1)
            MSEbbox = nn.MSELoss(reduction=red)
            lmse += MSEbbox(pxiyi, tbox[i][:, 4:12])

            # 给正样本的tobj赋初值，初值里用到了giou
            tobj[b, a, gj, gi] = (1.0 - model.gr) + model.gr * giou.detach().clamp(0).type(tobj.dtype)  # giou ratio

            if model.nc > 1:  # cls loss (only if multiple classes)  # 类别大于1 多分类
                t = torch.full_like(ps[:, 11:], cn)  # targets
                t[range(nb), tcls[i]] = cp
                lcls += BCEcls(ps[:, 11:], t)  # BCE  # 这个算的是类别的loss值
                # lcls += CE(ps[:, 5:], tcls[i])  # CE
        ...
        # 计算交叉熵 正样本与特征图上提的特征计算交叉熵 这个算的是置信度的loss值
        lobj += BCEobj(pi[..., 10], tobj)  # obj loss
    ...
    loss = lbox + lmse + lobj + lcls
    return loss, torch.cat((lbox, lmse, lobj, lcls, loss)).detach()
...
def build_targets(p, targets, model):
    # targets = [image, class, x, y,（归一化后的中心） w, h（归一化后的宽高）]   # image表示batch中图片编号 class表示类别 x y w h就是box信息
    nt = targets.shape[0]   #一个batch中target总数
    tcls, tbox, indices, av = [], [], [], []
    reject, use_all_anchors = True, True
    gain = torch.ones(6, device=targets.device)  # normalized to gridspace gain   # 在targets存放的设备上产生6*1的矩阵
    ...
    for i, j in enumerate(model.yolo_layers):
        ...
        # i值从0到2，对应尺度从大到小，cfg中读取文件后，0层除32，1层除16，2层除8，获得anchors值在当前层特征图上的尺寸(anchors值对应的是原图上的坐标)
        # anchors (3*2)
        anchors = model.module.module_list[j].anchor_vec if multi_gpu else model.module_list[j].anchor_vec

        # iou of targets-anchors
        # p[i]就是某层的预测结果，大小：16 * 3 * 13 * 13 * 15
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain 将该层的特征图的w h w h依次放在 gain的[2 3 4 5]位置
                                                           # torch.tensor(p[i].shape) 生成张量p[i].shape
        t, a = targets * gain, []  # t存放真值在特征图上的box信息 包括中心点坐标 宽 高坐标(相对于特征图的坐标) [image, class, x, y,（相对于特征图的中心） w, h（相对于特征图的宽高）]
                                   # *为对应位置元素相乘
        gwh = t[:, 4:6]  # 存放真值在特征图上的box宽高 gw gh
                         # gwh取t的第4-5列，即w，h
        if nt:
            ...
            if use_all_anchors:
                ...
                a = torch.arange(na).view(-1, 1).repeat(1, nt).view(-1)  # 每个真值对应的anchor编号
                t = t.repeat(na, 1)  # 每个真值在对应anchor上的类别信息以及box框的信息值(对应于当前层特征图的) [image, class, x, y, w, h]
            ...
            if reject:
                j = iou.view(-1) > model.hyp['iou_t']  # iou threshold hyperparameter
                                                       # j中存的是 [true / false] 是每个anchor与每个真值的一一对应关系
                t, a = t[j], a[j]  # 滤除阈值小于ignore thresh的anchor
                                   # t存的是真值在特征图上的box信息以及图像和类别信息，a是anchor的编号信息

        # Indices
        # 做完阈值滤除后筛选剩下的真值与anchor对应，其实就是正样本  # b是图像编号 c是真值的类别编号
        ...
        # Box
        gxy -= gxy.floor()  # xy  # 下取整 gxy算的是box框在特征图的栅格中的坐标值 是浮点的
    ...
    # indices存放的是上面写了  av存放的是正样本对应的不同层的anchor的box的宽高值（anchor真值除32  16  8）
    return tcls, tbox, indices, av


def build_targets_4p(p, targets, model):
    # targets = [image, class, x, y,（归一化后的中心） w, h,（归一化后的宽高） 4 * (xi, yi) (四个角点的坐标)]   # image表示batch中图片编号 class表示类别 x y w h就是box信息
    nt = targets.shape[0]   #一个batch中target总数
    tcls, tbox, indices, av = [], [], [], []
    reject, use_all_anchors = True, True
    gain = torch.ones(14, device=targets.device)  # normalized to gridspace gain   # 在targets存放的设备上产生6*1的矩阵
    ...
    for i, j in enumerate(model.yolo_layers):
        ...
        # iou of targets-anchors
        # p[i]就是某层的预测结果，大小：16 * 3 * 13 * 13 * 15
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 2]]  # xyxy gain 将该层的特征图的w h w h依次放在 gain的[2 3 4 5]位置
                                                           # torch.tensor(p[i].shape) 生成张量p[i].shape
        t, a = targets * gain, []  # t存放真值在特征图上的box信息 包括中心点坐标 宽 高坐标(相对于特征图的坐标) [image, class, x, y,（相对于特征图的中心） w, h（相对于特征图的宽高）]
                                   # *为对应位置元素相乘
        gwh = t[:, 4:6]  # 存放真值在特征图上的box宽高 gw gh
        ...
        gxy = t[:, 2:4]  # grid x, y  # box在特征图上的中心坐标
        gwh = t[:, 4:6]  # grid w, h  # box在特征图上的框的宽高
        gxiyi = t[:, 6:14] # box在特征图上的 4 * (xi, yi)
        # 网格索引，注意这里通过long将其转化为整形，代表格子的左上角
        gi, gj = gxy.long().t()  # grid x, y indices
        # indice结构体保存内容为：
        '''
            b: 一个batch中的下标
        ...
        # Box
        gxy -= gxy.floor()  # xy  # 下取整 gxy算的是box框在特征图的栅格中的坐标值 是浮点的
        tbox.append(torch.cat((gxy, gwh, gxiyi), 1))  # xywh (grids)  # tbox存的是正样本box在特征图栅格内的浮点坐标,box框的宽高值,以及4 * (xi, yi)
        av.append(anchors[a])  # anchor vec
    ...
    # tcls yolov3的三层中存下来的正样本的类别号，tbox 正样本存下来的在对应特征图上的相对自己栅格的box中心浮点坐标以及对应的box框, 以及4 * (xi, yi)
    # indices存放的是上面写了  av存放的是正样本对应的不同层的anchor的box的宽高值（anchor真值除32  16  8）
    return tcls, tbox, indices, av
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    method = 'merge'
    nc = prediction[0].shape[1] - 11  # number of classes
    multi_label &= nc > 1  # multiple labels per box
    output = [None] * len(prediction)

    for xi, x in enumerate(prediction):  # image index, image inference
        px = torch.cat((x[:, 0:1] - x[:, 2:3], x[:, 0:1] + x[:, 4:5], x[:, 0:1] + x[:, 6:7], x[:, 0:1] - x[:, 8:9]), 1)
        py = torch.cat((x[:, 1:2] - x[:, 3:4], x[:, 1:2] - x[:, 5:6], x[:, 1:2] + x[:, 7:8], x[:, 1:2] + x[:, 9:10]), 1)
        pxmax, index1 = torch.max(px, 1)
        pxmin, index2 = torch.min(px, 1)
        pymax, index3 = torch.max(py, 1)
        pymin, index4 = torch.min(py, 1)
        pwh = torch.cat((pxmax.view(-1, 1) - pxmin.view(-1, 1), pymax.view(-1, 1) - pymin.view(-1, 1)), 1)
        x = torch.cat((x[:, 0:2], pwh, x[:, 10:]), 1)
